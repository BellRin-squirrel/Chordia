...
sync_desc = "Chordia Syncは、複数のデバイスでご利用中のChordiaアプリの作業セッション履歴、楽曲再生履歴をオンラインで管理し、デバイス間で共有できるようにする機能です。Desktop版からMobile版へ楽曲を同期する機能とは異なります。"
btn_start_sync_auth = "Chordia Sync の認証をする"
sync_username_label = "ユーザー名"
sync_username_ph = "アカウント名を入力..."
sync_username_desc = "ユーザー名はChordia Syncのアカウント登録時に作成したアカウント名のことです。"
sync_device_label = "ログインデバイス名"
sync_device_ph = "例: Main Workstation PC"
sync_device_desc = "ログインデバイス名はログイン成功後にChordia Syncのデバイス一覧に表示されるこのデバイスの名前です。"
btn_auth_web = "ウェブで認証"
